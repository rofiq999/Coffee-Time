...
const search = (queryparams, hostAPI) => {
    return new Promise((resolve, reject) => {
        ...
        if (queryparams.sorting == "favorite") {
            query = "select pr.*, COALESCE(sum(tr.qty),0) as sold from product pr left join transactions tr on pr.id = tr.product_id where pr.delete_at is null GROUP BY pr.id ORDER by sold desc";
            link += `sorting=${queryparams.sorting}&`
        }
